...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11",jal ,     J , R(dest)=s->pc+4;s->dnpc=(s->pc + imm));
	...
	INSTPAT("0100000 ????? ????? 000 ????? 01100 11",sub,      R,  s_src1 = (int)src1;s_src2 = (int)src2;R(dest)=s_src1-s_src2;);
	...
	INSTPAT("0100000 ????? ????? 101 ????? 01100 11",sra,     R,  s_src1 = (int)src1;R(dest)=src1>>src2);
	INSTPAT("0000000 ????? ????? 001 ????? 01100 11",sll,     R,  R(dest)=src1<<src2);
	INSTPAT("0000000 ????? ????? 110 ????? 01100 11",or,     R,  R(dest)=src1 | src2);
	//INSTPAT("0100000 ????? ????? 101 ????? 00100 11",srai,     I,  u8=(uint8_t)(src1>>shamt);if((u8&0x80)==0x80){R(dest)=(u8 | 0xffffff00);}else{R(dest)=u8;});
	INSTPAT("0100000 ????? ????? 101 ????? 00100 11",srai,     I,  s_src1 = (int)src1;R(dest)=s_src1>>shamt);
	INSTPAT("??????? ????? ????? 001 ????? 01000 11",sh,     S,  Mw(src1 + imm, 2, src2));
	INSTPAT("??????? ????? ????? 101 ????? 11000 11",bge,     B,  s_src1 = (int)src1;s_src2 = (int)src2;if(s_src1>=s_src2){s->dnpc=(s->pc)+imm;});
	INSTPAT("??????? ????? ????? 100 ????? 00100 11",xori,     I,  R(dest)=src1^imm);
	INSTPAT("0000000 ????? ????? 111 ????? 01100 11",and,     R,  R(dest)=src1 & src2);
	INSTPAT("0000001 ????? ????? 000 ????? 01100 11",mul,     R,  s_src1 = (int)src1;s_src2 = (int)src2;R(dest)=s_src1 * s_src2);
	INSTPAT("0000000 ????? ????? 100 ????? 01100 11",xor,     R,  R(dest)=src1 ^ src2);
	INSTPAT("0000001 ????? ????? 100 ????? 01100 11",div,     R,  s_src1 = (int)src1;s_src2 = (int)src2;R(dest)=s_src1 / s_src2);
	INSTPAT("0000001 ????? ????? 110 ????? 01100 11",rem,     R,  s_src1 = (int)src1;s_src2 = (int)src2;R(dest)=s_src1 % s_src2);
	INSTPAT("??????? ????? ????? 100 ????? 11000 11",blt,     B,  s_src1 = (int)src1;s_src2 = (int)src2;if(s_src1<s_src2){s->dnpc=(s->pc)+imm;});
	INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = (int)Mr(src1 + imm, 2));
	INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu     , I, R(dest) = Mr(src1 + imm, 2));
	INSTPAT("0000000 ????? ????? 010 ????? 01100 11",slt,     R,  if(src1<src2){R(dest)=1;}else{R(dest)=0;});
